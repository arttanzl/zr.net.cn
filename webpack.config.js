...
var config = {
    ...
    module: {
        rules: [ //加载器，关于各个加载器的参数配置，可自行搜索之。
          ...
          },
          {
            //图片文件
        			test: /\.(png|jpg|gif)$/,
        			use: ['url-loader?limit=8192&name=img/[name].[ext]?v=[hash]']
          },
          {
            //字体文件
        			test: /\.(eot|woff|ttf)$/,
        			use: ['url-loader?limit=8192&name=font/[name].[ext]?v=[hash]']
        		}
        ]
    },
  ...
  plugins: [
    ...
    //   }
    // }),
    new ExtractTextPlugin('css/[name].css?v=[hash]'), //单独使用link标签加载css并设置路径，相对于output配置中的publickPath
